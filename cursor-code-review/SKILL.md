...
set -e
N=123
read -r base_ref base_oid head_oid < <(
...
cursor-agent -p --output-format text --mode plan --trust \
  ...
  "Review git diff $base_oid...$head_oid for PR #$N. Report only concrete defects introduced by the change, with severity, file:line, failure scenario, and fix. Make no changes and no GitHub calls." \
  >"$review_file"
```
