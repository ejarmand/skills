...
3. **Branch — is this a multi-session build?**
   ...
   Either way, **`/implement`** builds each issue by driving **`/tdd`** internally — one red-green slice at a time, written against the unified standard — then runs focused checks before commit. Reach for **`/tdd`** on its own when you just want to build a concrete behaviour test-first, and **`/code-review`** whenever you want to review a branch or PR against a fixed point.
