...
3. **Branch — is this a multi-session build?**
   ...
   Either way, **`/implement`** builds each issue by driving **`/tdd`** internally — one red-green slice at a time — then invokes **`/cleanup-abstraction`** on the green diff, runs focused checks, and closes out with **`/code-review`**, a two-axis review (Standards + Spec), before full checks and commit. Reach for **`/tdd`** on its own when you just want to build a concrete behaviour test-first, **`/cleanup-abstraction`** when an already-working path has accumulated indirection, and **`/code-review`** whenever you want to review a branch or PR against a fixed point.
