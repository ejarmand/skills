...
exactly one parent-visible shell call, launch their `/codex-agent` commands in
an `xargs -0 -n 1 -P "$parallelism"` pool. Every worker uses
`gpt-5.6-luna` at `high` reasoning effort.

Validate `parallelism` as a canonical positive decimal matching
`^[1-9][0-9]*$` before calling `xargs`; zero, zero-padded values, non-numbers,
and `-P 0` are invalid.
...
Record each worker's status and result under its job directory without letting
one worker stop the remaining jobs from launching. After `xargs` has waited for
the whole pool, emit every stable label with either its validated
`/codex-agent` result or a visible failure, and return nonzero if the pool or
any worker failed.
